...
func main() {
	...
	for i, _ := range tx.RctSignature.OutPk {
		var str string
		str = fmt.Sprintf("tx1.RctSignature.OutPk[%d]: %x", i, tx.RctSignature.OutPk[i])
		noty.NotifyWithLevel(str, LevelInfo)
		str = fmt.Sprintf("tx2.RctSignature.OutPk[%d]: %x", i, tx2.RctSignature.OutPk[i])
		noty.NotifyWithLevel(str, LevelInfo)
	}
